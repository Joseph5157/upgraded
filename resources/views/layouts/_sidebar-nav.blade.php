...
<div class="px-5 py-5 flex items-center justify-between" style="border-bottom: 1px solid rgba(255,255,255,0.06);">
    ...
    <button onclick="closeSidebar()"
            class="md:hidden w-7 h-7 rounded-lg flex items-center justify-center transition-colors"
...
<nav class="flex-1 px-3 py-5 space-y-1 overflow-y-auto">
    <p class="text-[9px] font-bold uppercase tracking-widest px-3 mb-3" style="color:#475569;">Navigation</p>

    @php $isDashboard = request()->routeIs('dashboard'); @endphp
    <a href="{{ route('dashboard') }}"
        class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium {{ $isDashboard ? 'active' : '' }}"
        style="color: {{ $isDashboard ? '#a5b4fc' : '#94a3b8' }};">
        <svg class="w-4 h-4 flex-shrink-0" style="color: {{ $isDashboard ? '#818cf8' : '#64748b' }};"
    ...
    @php $isEarnings = request()->routeIs('vendor.earnings'); @endphp
    <a href="{{ route('vendor.earnings') }}"
        class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium {{ $isEarnings ? 'active' : '' }}"
        style="color: {{ $isEarnings ? '#a5b4fc' : '#94a3b8' }};">
        <svg class="w-4 h-4 flex-shrink-0" style="color: {{ $isEarnings ? '#818cf8' : '#64748b' }};"
            fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="pt-4 mt-2" style="border-top: 1px solid rgba(255,255,255,0.06);">
        <p class="text-[9px] font-bold uppercase tracking-widest px-3 mb-3" style="color:#475569;">Account</p>

        @php $isProfile = request()->routeIs('profile.*'); @endphp
        <a href="{{ route('profile.edit') }}"
            class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium {{ $isProfile ? 'active' : '' }}"
            style="color: {{ $isProfile ? '#a5b4fc' : '#94a3b8' }};">
            <svg class="w-4 h-4 flex-shrink-0" style="color: {{ $isProfile ? '#818cf8' : '#64748b' }};"
                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
<div class="px-4 py-4" style="border-top: 1px solid rgba(255,255,255,0.06);">
    <div class="flex items-center gap-3">
        <div class="w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold flex-shrink-0"
            style="background:rgba(79,70,229,0.2); color:#a5b4fc; border:1px solid rgba(79,70,229,0.2);">
        ...
        <form method="POST" action="{{ route('logout') }}" class="ml-auto flex-shrink-0">
            @csrf
            <button type="submit" class="transition-colors" style="color:#64748b;" title="Log out"
