<x-admin-layout>
    ...
    @php $csvUrl = route('admin.finance.reports.vendor-payouts.csv') . '?' . http_build_query(array_filter(request()->only(['from','to','vendor_id','payment_mode','status']))); @endphp
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <h1 class="text-lg font-bold text-gray-900 dark:text-white tracking-tight">Vendor Payouts Report</h1>
            <p class="text-[10px] text-gray-400 dark:text-slate-500 uppercase tracking-[0.25em] mt-0.5 font-mono">{{ $payouts->total() }} records · Total: ₹{{ number_format($total, 2) }}</p>
        </div>
    ...
    {{-- Filter form --}}
    <form method="GET" action="{{ route('admin.finance.reports.vendor-payouts') }}" class="bg-white dark:bg-[#0d0d10] border border-[#E8ECF0] dark:border-white/[0.05] rounded-2xl p-5 mb-6 flex flex-wrap items-end gap-3">
        <div>
        ...
        <div>
            ...
            <select name="payment_mode" class="bg-[#F5F7FA] dark:bg-white/[0.04] border border-[#E8ECF0] dark:border-white/[0.08] rounded-xl px-3 py-2 text-xs text-gray-900 dark:text-white focus:outline-none">
                ...
                @foreach(['upi','bank_transfer','cash'] as $mode)
        ...
        @if(array_filter([$filters['from'],$filters['to'],$filters['vendor_id'],$filters['payment_mode'],$filters['status']]))
            <a href="{{ route('admin.finance.reports.vendor-payouts') }}" class="px-4 py-2 text-gray-400 text-[9px] font-bold uppercase tracking-widest rounded-xl border border-gray-200 dark:border-white/[0.08] transition-all">Clear</a>
        @endif
    </form>
                ...
                <table class="w-full text-left text-xs">
                    <thead class="text-[9px] text-gray-400 dark:text-slate-500 font-bold uppercase tracking-[0.2em] border-b border-gray-100 dark:border-white/[0.04]">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-center">Paid By</th>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/[0.04]">
                        @foreach($payouts as $p)
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                                ...
                                <td class="px-4 py-2.5 font-bold text-gray-900 dark:text-white">{{ $p->vendor?->name ?? '—' }}</td>
                                <td class="px-4 py-2.5 text-right font-mono text-blue-500 font-bold">₹{{ number_format($p->amount,2) }}</td>
                                <td class="px-4 py-2.5 text-center font-mono text-gray-500 dark:text-slate-400 uppercase">{{ str_replace('_',' ',$p->payment_mode??'—') }}</td>
                                <td class="px-4 py-2.5 text-center font-mono text-gray-400 dark:text-slate-500 text-[9px]">{{ \Illuminate\Support\Str::limit($p->reference_id??'—',12) }}</td>
                                <td class="px-4 py-2.5 text-center">
                                    <span class="px-1.5 py-0.5 text-[9px] font-bold uppercase rounded-lg {{ $p->status==='paid' ? 'text-green-400 bg-green-500/10' : 'text-red-400 bg-red-500/10' }}">{{ $p->status }}</span>
                                </td>
                                <td class="px-4 py-2.5 text-center text-gray-500 dark:text-slate-400">{{ $p->paidBy?->name ?? '—' }}</td>
                                <td class="px-4 py-2.5 font-mono text-gray-500 dark:text-slate-400">{{ $p->paid_at?->format('d M Y') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
            <div class="px-5 py-3 border-t border-gray-100 dark:border-white/[0.04]">{{ $payouts->links() }}</div>
        @else
            <div class="py-16 text-center"><p class="text-sm text-gray-400 dark:text-slate-500">No payouts match the selected filters.</p></div>
        @endif
    </div>
